...
function test_biochemical_model()
  ...
  for index in 1:number_of_subdivisions

    # Run POETs -
    (EC, PC, RA) = estimate_ensemble(objective_function, neighbor_function,
      acceptance_probability_function, cooling_function, initial_parameter_array;
      rank_cutoff=4, maximum_number_of_iterations=20, show_trace=true)

    # Package -
    ec_array = hcat(ec_array, EC)
    pc_array = hcat(pc_array, PC)

    # Take the *best* value from the current ensemble, refine it and go around again -
    total_error = sum(ec_array[:, 2:end], dims=1)

    # Which col is the min error?
    min_index = argmin(vec(total_error))
