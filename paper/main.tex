...
The algorithm is entirely defined by four user-supplied callback functions (\cref{tab:callbacks}).
The objective function maps a parameter vector $\mathbf{x} \in \mathbb{R}^d$ to an $m \times 1$ array of objective values; the neighbor function perturbs a given parameter vector to generate a candidate, with multiplicative Gaussian noise and bound enforcement being a common choice; the acceptance probability function takes the current rank array and temperature and returns a scalar probability, typically of the Boltzmann form $\exp(-R_{\text{last}} / T)$; and the cooling function maps the current temperature to a reduced value, with geometric schedules of the form $T' = \alpha T$ for $\alpha \in [0.8, 0.95]$ providing a practical balance between exploration and convergence.
Beyond the four callbacks, the behavior of \texttt{estimate\_ensemble} is controlled by several keyword arguments: \texttt{rank\_cutoff} (default~5) sets the pruning threshold, \texttt{maximum\_number\_of\_iterations} (default~20) controls candidates per temperature level, \texttt{temperature\_min} (default $10^{-4}$) sets the stopping criterion, \texttt{maximum\_archive\_size} (default~1000) provides a hard upper bound on archive size, and \texttt{parallel\_evaluation} activates threaded ranking.
...
Constraints in the Binh--Korn problem are enforced via quadratic penalty terms added to the objective values, and bound constraints are handled by clamping in the neighbor function; representative Julia code for both the single-chain and multi-chain interfaces is provided in the Supplementary Material (\cref{lst:binhkorn,lst:parallel}).
...
ParetoEnsembles.jl achieved comparable or slightly higher hypervolume than NSGA-II on both problems and substantially lower IGD --- 5$\times$ lower on Binh--Korn and 7$\times$ lower on Fonseca--Fleming --- indicating a closer approximation to the true Pareto front (\cref{fig:comparison}).
The SA-based approach also produced much denser fronts, retaining 2{,}000--6{,}000 non-dominated solutions compared with 200 for NSGA-II, reflecting its design as an ensemble generator that characterizes the full cloud of near-optimal solutions rather than merely approximating the front boundary.
NSGA-II was 50--150$\times$ faster in wall time, consistent with its population-parallel evaluation strategy, but this speed advantage comes at the cost of a sparser representation of the trade-off surface.
These results confirm that ParetoEnsembles.jl is competitive on front quality while offering the additional benefit of a dense near-optimal ensemble suitable for downstream uncertainty analysis in applications such as parameter estimation and model prediction.
...
The hyperparameter sensitivity analysis further shows that the algorithm is robust to the choice of rank cutoff, cooling rate, and iterations per temperature, so that users can adopt the default settings without extensive tuning.
The convergence trace feature, activated via the \texttt{trace} keyword argument, provides a built-in diagnostic for monitoring archive growth and hypervolume evolution over the annealing schedule, helping users assess whether the algorithm has run long enough for their particular problem.
...
% ============================================================
% TABLES, FIGURES, AND ALGORITHMS (after references for journal submission)
% ============================================================
\clearpage
